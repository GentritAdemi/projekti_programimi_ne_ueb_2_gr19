...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
  ...
  while ($row = mysqli_fetch_assoc($allquery)) {
    echo "<div class='image-container'>
            <img class='image' src='{$row['image_url']}' alt='Movie Image'>
            <div class='overlay'>
              <span class='play-button'></span>
              <h3 class='movie-title'>{$row['tmovie_tittle']}</h3>
              <p class='movie-kind'>{$row['movies_kind']}</p>
            </div>
          </div>";
  }
  ?>
  <?php
  if($search!=""){
  $query = "SELECT * FROM movies_post where tmovie_tittle = {$search}";
  $allquery = mysqli_query($connection, $query);
  while ($row = mysqli_fetch_assoc($allquery)) {
    echo "<div class='image-container'>
            <img class='image' src='{$row['image_url']}' alt='Movie Image'>
